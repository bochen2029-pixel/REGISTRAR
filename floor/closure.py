...
produced the number — so the output is a derivation a clinician can check in
seconds rather than an assertion. See SPEC.md §6.
...
CORRECTNESS DISCIPLINE
  ...
  INF + INF does not overflow a 32-bit integer. That is the classical choice
  and it is load-bearing — an accelerated implementation must produce
  BIT-IDENTICAL output to this one, asserted by equality and never by
  tolerance, or it cannot pass replay determinism.
...
def load_case(path: str) -> tuple[STN, dict]:
    """
    Load a case fixture. JSON rather than YAML so the floor runs on a bare
    ...
    for c in doc.get("constraints", []):
        ...
        if kind == "at_least":
            ...
            stn.at_most(c["event"], REFERENCE, int(c["minutes"]), label, layer)
            stn.at_least(c["event"], REFERENCE, int(c["minutes"]), label, layer)
        else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(__doc__)
        print("usage: python floor/closure.py <case.json> [now_minutes]")
        raise SystemExit(2)
