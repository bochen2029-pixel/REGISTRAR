#!/usr/bin/env python3
"""
REGISTRAR · floor · closure tests

Zero dependencies, no test framework. Run it:

    python floor/test_closure.py

These are not smoke tests. Each one asserts a property the design claims in
public, so that a claim on the website and a claim in the code cannot drift
apart silently.
"""
...
from closure import INF, REFERENCE, Closure, STN, hhmm, load_case  # noqa: E402

HERE = os.path.dirname(os.path.abspath(__file__))
FIXTURES = os.path.join(os.path.dirname(HERE), "fixtures", "cases")
...
def test_implied_deadline() -> None:
    """
    The claim published at opnaorta.ai/edr §11: given the six constraints of
    the morning-OR-window case, the closure derives that the serology had to
    be drawn by 22:15 the previous evening — a deadline no single field holds.

    22:15 the previous day is -105 minutes from the reference (midnight of the
    OR day). If this number ever changes, the website is wrong.
    """
    print("\nimplied deadline — the number on the website")
...
def test_negative_cycle_is_infeasibility() -> None:
    ...
    # A one-element "cycle" is a degenerate answer that tells a coordinator
    # nothing. The earlier version of this test accepted one; it should not.
...
def test_order_independence() -> None:
    ...
    network. This is the local shadow of the associativity the patch algebra
    relies on: order of authorship must not change the result.
    """
    print("\norder independence")
    stn, doc = load_case(os.path.join(FIXTURES, "morning-or-window.json"))
    forward = stn.close()
...
def test_floor_has_no_model() -> None:
    """
    The floor must behave identically with every learned component disabled,
    because there are none. This test exists to keep it that way: if an import
    of anything model-shaped appears in closure.py, this fails.
    """
    print("\nthe floor is model-free")
