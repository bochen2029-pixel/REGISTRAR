...
chunks swept into a public commit by the staging collision) **and the completion falsifier's spoilers** —
`DELTA.json`, the extractor and builder whose comments name every trap, and the arm-① result file. The
hygiene gates are structurally blind to history (they ask `git ls-files`, which sees only HEAD), so this
stood GREEN until the QC session probed `git show` against an old commit directly.

**Rewritten with `git filter-repo` and force-pushed while the repository had zero known external clones —
the cheapest moment there will ever be.** Six paths purged from all 69 commits; the working tree untouched
(the vault was already untracked, the smoke output already ignored). A full mirror backup was taken
immediately before the rewrite and is retained offline.

**The SHA translation is committed** at `provenance/history-rewrite-2026-08-26.commit-map` (old → new, one
line per commit). **And the one SHA that load-bears did not change:** the F-PATCH-DELTA pre-registration
pinned the gate battery at `87c463d`, which **predates the first purged commit and is therefore
byte-identical in the new history.** The pin survives verbatim; every later SHA translates through the map.

**Why rewriting history is consistent with an append-only culture:** this log records that it happened, what
was removed, and why. The erasure is of *redistributed third-party text and experiment spoilers* — never of
the record of our own conduct, which is this entry. **Stated limit:** force-push cannot purge server-side
caches GitHub may hold; with no forks and no PRs, no public ref reaches them, and that residual is named
here rather than assumed away.

**A defect inside this very fix, kept because it teaches:** the first version of this entry was written
through an unquoted shell heredoc, and the backticked prose *executed* — `git ls-files` ran and dumped a
file listing into the middle of the paragraph. The corrupted version was committed and force-pushed before
being caught one command later. This repair is the correction, and the lesson is already in the house laws:
**write files with the Write tool, never through heredocs.** It is now also the fourth time that lesson has
been paid for.
