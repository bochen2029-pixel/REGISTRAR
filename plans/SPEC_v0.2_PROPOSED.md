...
>
> **This is a PROPOSED v0.2 of `SPEC.md`, produced 2026-08-27 by mainline. The in-force specification is
> `SPEC.md` (v0.1) until an operator adopts this.** Method: the v0.1 text was copied byte-exact and amended
> surgically — every change is enumerated below with its reason, and everything not listed is untouched.
> Adoption is an explicit operator act: replace `SPEC.md` with this body (minus this banner block) in a
> commit that says so.
>
> ## Amendment log — eight changes, and the standard each had to meet
>
> A change earned its place only if the v0.1 text is now **false against a dated receipt** — not stale in
> emphasis, not improvable, *false*. The constitution gets amended for truth, never for taste.
>
> 1. A1 §header — 'PRE-BUILD' was true on 2026-08-25 and is false now: R1–R5 shipped with receipts. The no-OPO/no-PHI sentence is unchanged because it is still true.
> 2. A2 §2 — the patch contract gained `holds` (gate 15, adopted from a completion run's own shape) months of receipts ago; the spec's table never carried it. Also names the measured row-XOR-hold under-specification instead of leaving it implicit.
> 3. A3 §4 — the table predates gates 13–16 (divergence, schema conformance, accountability, attest); adds the floor-vs-semantic-gate distinction and the fact that battery strength is now a measurement.
> 4. A4 §5 — the bet paragraph still pointed at an unrun falsifier; it ran five times and the receipts (including the void and the fails) belong in the constitution's own text.
> 5. A5 §9(a) — 'unpinned, no assertable SHA' was true at banking and is false now: the pin exists and is re-verified continuously.
> 6. A5 §9(b) — the four-piece gap was closed to three narrower pieces by F-BOOT, with the install/build doctrine measured; and the pending batteries-included proposal is cross-referenced without being adopted.
> 7. A6 §11 — the ladder's R5 paragraph still read as pre-run advice; rung status with receipts recorded, R6 named as the only open rung.
> 8. A7 §12(a) — the falsifier bullet updated from 'printed' to 'run', with its bounds.
...
> smuggle itself into the constitution through a copy-edit).

---

# REGISTRAR — SPECIFICATION · v0.2 `[PROPOSED]`

**2026-08-27 · STATUS: BUILT AND MEASURED THROUGH R5 of §11; R6 open.** Nothing described here has run
inside an OPO. No patient data has touched any part of it.
...
| **Divergence** | a row whose three tellings disagree — what it *says* (`value`), what it *cites* (`evidence`), what *happened* (`shadow_run`); optimistic rounding refused, conservative rounding accepted because rounding up is the house rule |
| **Schema conformance** | a patch that does not validate against the shipped contract — a schema nothing validates against is a document, not a contract |
| **Accountability** | a patch silent on any declared target — a row or a hold, never nothing |
| **Attest** | evidence that, read as prose, does not support its value: describes something no longer in force, denies the bound the row asserts, or is permissive where the row is mandatory (`may` is not `must`) |

**Two structural facts about the battery, learned by building it.** First: `schema conformance` and
`accountability` are **floors, not peers** — they ask *is this a well-formed, complete patch at all*, fire on
any deliberately-minimal fragment by construction, and are therefore excluded from per-gate isolation
measurement (the witness matrix) while remaining fully live in the battery. Second: **the battery's strength
is measured, not asserted** — every gate carries a witness fixture or a named structural reason it cannot
have one, and mutation testing scores the battery against deliberately broken variants of itself.
`[M — witness matrix + mutation harness, experiments/F-BATTERY-STRENGTH]`
...
That is a far easier bet, and it is the one this project takes. **Its falsifier is R5 in §11 — and it has
now RUN, five times, wholly synthetic**: verdict v1 VOID-BY-AMBIGUITY (the rubric admitted two readings;
killed rather than resolved favourably) → rubric v2/v3 FAILS on the v1 corpus (whose honest ceiling, 0.575,
made the top band unreachable without fabricating — an instrument fact, found and fixed) → corpus v4 with
every answerable target given a replay surface: **`S = 0.66 ± 0.04 (n = 2)`, the no-material floor cleared
decisively in both runs, and the COVERS boundary unresolved at that sample size.** The invariant across every
run, every corpus, every rubric: **zero fabricated values, ever** — three independent fresh sessions, one
deliberately unanswerable target, not one invention. `[M — experiments/F-PATCH-DELTA/RESULTS.md]`
**Standing bound, undiminished:** a synthetic corpus bounds the kit from above; whether a *real* OPO is
legible is R6 and is not claimed.
...
**Stated precisely, because the tense here has been wrong twice now.** The chassis sits in this
repository's root, **pinned byte-for-byte** — `CHASSIS.pin.json`, 7,895/7,895 files verified against the
upstream tarball, re-checked by the battery and by every boot `[M]`. It is gitignored and a conformance gate
fails if it is ever staged, because **third-party code is referenced and verified, never shipped from an MIT
tree.** The algebra of §3 is implemented **directly**, in
...
**The gap, restated after F-BOOT `[M — 2026-08-27]`.** A clone now delivers a **standing harness in one
command**: `forge/boot.py` verifies the pin *before anything executes*, installs, and launches upstream's own
entry — HTTP answer on loopback from a pin-verified tree, `require_forge`-guarded so a deployed record can
never run it. Two doctrines were **measured, not argued**, in the process: **install is composition** — the
pin re-verifies byte-identical after it; **build is a fork** — upstream commits its artifacts, `pnpm run
build` added 6,463 files to the pinned tree (caught by the pin, removed byte-verified), *and the build is
unnecessary*: the harness runs from source via tsx, cold start ≈140 s measured.

**What remains of the gap, narrowed:** a REGISTRAR **profile** (the stock tree boots; a tree carrying only
what a record needs does not exist yet) · **level-2 plugin mounting** (`chunk` and `phi_scan` are subprocess
capabilities; in-harness tool registration is declared, not done) · **FUSOR mounted** (the percept stream and
switch exist repo-side; nothing mounts them into the harness). A packaged desktop chassis remains separate
and further out. **A doctrine decision is separately pending, proposed and not adopted:** reference bindings
for `read`/`reach` on the chassis pattern — *pin, don't vendor* — see `plans/HORIZONS_2026-08-27.md`.
...
The original three-outcome framing stands as written; the outcome drawn was the second, sharpened: the kit
completes a corpus *built to be completable*, and says nothing yet about a real OPO.
...
- The completion claim **has run its falsifier** (§5, §11): first receipts exist, they are synthetic-only,
  and the COVERS boundary is unresolved at n = 2. **The real-site claim remains unclaimed.**
...
- The central risk is not a wrong patch. It is a **weak battery** — because a foreign harness will produce
  ...
  where wrong loses an organ. **This risk is now measured rather than feared** — witness coverage per gate,
  mutation testing against broken variants of the battery itself, and six named exposures retained as
  fixtures that pass — **and a new risk earned its place beside it: doctrine drift under maintenance.** Four
  documented instances in two days, none deliberate, none caught by a check until the claims gate existed;
  numbers are now gated (`conformance/claims.py` — the plate is derived and surfaces are checked against it),
  prose emphasis is not, and the re-founding audit exists because of that.
...
*v0.2-PROPOSED, drafted 2026-08-27 against receipts through `f3c9986`. R1–R5 of the ladder are built and
measured; R6 is open and is not an engineering rung. The ladder in §11, not this document, decides what
happens next — and where this document and a dated receipt disagree, the receipt wins.*
